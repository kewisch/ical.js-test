...
async function createXpcshellManifest(buildPath, testPath, { manifest, includeLightningHeads=false, xpcshellTestPaths=[] }) {
  ...
  if (xpcshellTestPaths) {
    content += xpcshellTestPaths.map(addlTestPath => {
      return `[include:${path.relative(buildPath, path.join(testPath, "xpcshell/tests", addlTestPath))}]`;
    }).join("\n");
  }

  if (manifest) {
    content += `[include:${path.relative(buildPath, manifest)}]\n`;
  }
...
async function main() {
  ...
  let xpcshellTestPaths = core.getInput("xpcshellTestPaths");
  ...
  let actions = new Set(core.getInput("actions").split(/,\s*/));

  let testTypes = ["common"];
  if (xpcshell || xpcshellTestPaths) {
  ...
  if (actions.has("run") && (xpcshell || xpcshellTestPaths)) {
    ...
    let manifest = await createXpcshellManifest(buildout, testPath, {
      ...
      includeLightningHeads: core.getInput("lightning") == "true",
      xpcshellTestPaths: core.getInput("xpcshellTestPaths").split(/,\s*/)
    });

    let check = new CheckRun("xpcshell", github.context, process.env.GITHUB_TOKEN);
    await check.create();

    core.startGroup("xpcshell tests verbose");
    try {
